...
class FrmMisCitas extends Component {
  constructor() {
    ...
    this.handleOnCloseCitaCancelada = this.handleOnCloseCitaCancelada.bind(
      this
    );
    this.handleOnclickAceptarCancelacion = this.handleOnclickAceptarCancelacion.bind(
      this
    );
  ...
  async handleOnClick(e, _idSesion, _idTutor, _fecha, _idProg) {
    ...
    if (facu) {
      let pol = await Controller.GET({
        servicio: "/api/facultad/politicas/" + facu.facultad.ID_FACULTAD,
      });
    ...
    let fechaConAnticip = moment(new Date())
      .add(this.state.diasAnticipacion, "days")
      .format("YYYY-MM-DD");
  ...
  async componentDidUpdate(prevProps, prevState) {
    if (prevState.actuTys != this.state.actuTys) {
      let arregloDeSesiones = await Controller.GET({
        servicio: "/api/listaSesionAlumno/" + getUser().usuario.ID_USUARIO,
      });
      ...
      for (let element of arregloDeSesiones.data) {
        ...
        arreglillo.push({
          ...
          nombre: element.TUTOR
            ? element.TUTOR.USUARIO.NOMBRE +
              " " +
              element.TUTOR.USUARIO.APELLIDOS
            : "",
            ...
            <Button
              ...
              onClick={(e) =>
                this.handleOnClick(
                  ...
                  element.ID_TUTOR,
                  element.FECHA,
                  element.PROCESO_TUTORIum.ID_PROGRAMA
                )
              }
          ...
          campoEstado:
            estadillo !== "00" && estadillo !== "01" && estadillo !== "02"
      ...
      <div>
            ...
            <DialogContent>
              <Grid container md={12} justify="center">
                <Typography variant="subtitle1">
                  Por política de la facultad solo puede cancelar su cita con
                  mínimo {this.state.diasAnticipacion} días de anticipación
                </Typography>
              </Grid>
            </DialogContent>
            <DialogActions>
        ...
        <TabProceso
          procesos={[ 
            {
              index: 0,
              titulo: "Pendientes", //Pendientes y realizadas
              proceso: () => (
                <TablaTutoresMisCitas
