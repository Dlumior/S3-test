...
function Tabla(props) {
  ...
  if (valeria === "PyR") {
    ...
    props.sesiones.columns.forEach((element) => {
      if (element.title !== "Motivo Cancelación") {
    ...
    props.sesiones.columns.forEach((element) => {
      if (
        ...
        element.title !== "Motivo Solicitud" &&
        element.title !== "Estado" &&
        element.title !== "Reprogramar Cita" &&
  ...
  props.sesiones.data.forEach((element) => {
    ...
    if (props.estado === "PyR") {
      ...
      //"Cancelada" como parametro padre
      if (element.campoEstado === "Cancelada") {
        // "Cancelada" como nombre en el campoEstado
        element.campoCont = i;
        arr2.push(element);
        i++;
      }
